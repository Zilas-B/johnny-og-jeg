// One-shot seed script for the homePage singleton (Step 7e: block-composed).
...
//
// Writes the published `homePage` doc directly (non-draft _id → published;
// createOrReplace keeps re-runs idempotent), then clears any stale old-shape
// `drafts.homePage` left from the pre-7e named-field schema so Studio shows the
...
async function main() {
  ...
  // Clear any stale pre-7e draft (old named-field shape) so Studio shows the
  // clean published block-composed document. Idempotent: ignore "not found".
  try {
    await client.delete('drafts.homePage')
    console.log('Deleted stale draft: drafts.homePage')
  } catch (err) {
    if (err?.statusCode === 404) console.log('No stale draft to delete.')
    else throw err
  }
}
