...
| Styling         | **CSS Modules + global tokens**                     | One `.module.css` per component for everything: layout, spacing, colour, the bespoke editorial pieces (drop caps, paper grain, vinyl spin, per-page accent overrides). `styles/tokens.css` holds the design variables, `styles/reset.css` the hand-written reset. No utility framework — Tailwind was removed in ADR 0005.                                                                                                                                                                                                                                                                            |
...
| Formatting      | Not configured yet                                  | Prettier was planned but not installed at scaffold time. Add `prettier` when consistency starts to matter.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Testing         | **Playwright** viewport smoke tests only            | `pnpm test:viewport`, pinned `@playwright/test`, Chromium only. Runs every route in `/sitemap.xml` at 320 / 375 / 768 / 1280 px, asserts the document never scrolls horizontally, and saves a screenshot per route × width as an artefact for a human to look at. Per-area specs additionally assert 44×44px touch targets (WCAG 2.5.8) on the interactive elements their own ticket covers — not a sitemap-wide sweep like the overflow check. A small `unit` project covers the sitemap parser. **No pixel-diff baselines, no broad E2E, no unit tests of presentational components.** Amended in #29: a chrome component whose acceptance criteria are behavioural — focus trap, Escape, scroll lock, `aria-expanded` — may have its own interaction spec in `tests/viewport/`, because a screenshot cannot see any of them. `tests/viewport/nav-drawer.spec.ts` is the pattern: one component, criteria only, skipped at the widths it does not apply to. Widened in #31 from chrome to any component, and from behavioural criteria to layout ones — "the three Vinyl Tiles stack in one column" is no more visible to a screenshot than a focus trap is. The pattern is unchanged and the limits still bind: a spec exists only for a ticket's own acceptance criteria, asserts them and nothing else, and skips the widths they do not apply to. `tests/viewport/home-blocks.spec.ts` is the first layout spec. ADR 0002's Vitest test of the contact route handler is still to come and is not superseded by this row. CI runs it blocking (#40): a failing overflow or touch-target assertion fails the build. |
...
├── styles/
│   ├── globals.css                   # Imports tokens + reset; body base styles, skip-link
│   ├── reset.css                     # Hand-written reset; the site's only baseline (ADR 0005)
...
Established in #28. Binding for every new rule.

**Breakpoints are desktop-down.** Base rules describe the widest layout; every
`@media` narrows it with `max-width`. Three steps, and only these three:

| Breakpoint            | Intent                                                 |
|-----------------------|--------------------------------------------------------|
| `(max-width: 1024px)` | Multi-column grids collapse toward one or two columns. |
| `(max-width: 768px)`  | Tablet portrait; side-by-side pairs stack.             |
| `(max-width: 560px)`  | Phone; the last horizontal splits go vertical.         |

Mobile-first `min-width` was rejected: converting would mean rewriting the six
existing rules plus every module they touch, for a stylistic preference.

**Minimum width is 360px.** Layouts are not tuned below it. The viewport suite
runs 320 / 375 / 768 / 1280 — it does not test 360 itself, and 320px is the
guard rail: it must eventually stop scrolling horizontally, but as of #28 it
still does on every route (`docs/responsive-overflow.md`, the residual list
#29–#39 work from).

**The content container comes from `--gutter` and `--container-max`**
(`styles/tokens.css`); `--gutter` steps down at each breakpoint. Three separate
`.wrap` rules exist — `tokens.css`, `Nav.module.css`, `PullQuote.module.css` —
because CSS Modules scope them apart, and a fix in one does not reach the other
two. All three read the tokens, so never hardcode a container width or side
padding, and add a fourth copy only by reading the tokens too. This paragraph is
the canonical statement of that; the CSS comments point here rather than repeat it.

**Display type is fluid, body type is not.** Every `font-size` of 32px or more is
`clamp(min, Xvw, max)`, generated by one formula:

- `max` — the original design size, reached at a **1280px viewport** (not the
  1280px container, which is narrower once the gutter is subtracted).
- `Xvw` — `max ÷ 12.8`, so the type is linear in viewport width.
- `min` — `20px + 0.2 × max`, rounded. Large type shrinks proportionally further
  than small type, and nothing ever falls into body-text range.

The formula is the default, not a law: a heading may be hand-tuned to a different
`clamp()` when a designer decision says so. Say why in a comment when you do.

Body copy, mono labels and kickers stay fixed px and are adjusted inside
breakpoints when they need to be. A `clamp()` and a `@media` `font-size` override
for the same element must not coexist — the clamp wins the argument.

## Versioning policy

- Pin Next.js and Sanity major versions in `package.json`.
...
- No CSS-in-JS runtime (styled-components, emotion) — CSS Modules cover everything.
