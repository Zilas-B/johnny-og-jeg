...
const draftClient = readToken ? client.withConfig({ token: readToken }) : null
...
export async function GET(request: Request) {
  if (!handler) {
    return new Response(
      'Missing SANITY_API_READ_TOKEN — set it locally in .env.local (and in Vercel for production) to use the Presentation tool.',
      { status: 500 },
    )
  }
